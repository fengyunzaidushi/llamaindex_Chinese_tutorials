...
def get_first_folder_files(directory):
    ...
    folder_dict = {}
    first_dirs = list_subdirectories(directory)
    for folder in first_dirs:
        files = get_filelist(directory + "/" + folder)
        folder_dict[folder] = {'nums': len(files), 'files': files}

    new_dict = sorted(folder_dict.items(), key=lambda x: x[1]['nums'], reverse=True)
    for folder, nums in new_dict:
        print(f'folder {folder} has {nums["nums"]} files')
